...
| `initTabGroup(...)` | タブ切替。**親タブ(記事/スタンプ/ゲーム/動画/掲示板)と媒体サブタブで共用** |
| `initMediaVisibility` | 媒体サブタブは記事タブ専用なので、他タブでは隠す |
| `initFilter` | 検索と絞り込み |
| `initNoteView` | note記事タブ内の表示切替（時系列/マガジン別/有料記事）。下記参照 |
...
### noteタブは時系列/マガジン別/有料記事を切り替えられる（2026-09-05〜）

記事タブ内のnote媒体パネルは、以前は「有料記事」「AIで遊ぼう編」「note.com公開記事」という
手動区切りの3セクションだったが、**現在は全55記事がフラットな1グリッドに時系列で並んでいて、
上部のトグルで時系列/マガジン別/有料記事の3表示に切り替える**方式に統一した。

- マガジン別は note.com 実物の**7マガジン**に対応するチップで絞り込む（`data-magazine`属性の一致判定）
- カードは重複させない。1記事1カードのみで、絞り込みは表示/非表示（`is-note-hidden`クラス）で行う
  （マガジン別に全部並べる案は不採用。複数マガジン所属の記事があり、件数カウントが壊れるため）
- 新しいnote記事を足すときは `data-magazine` 属性の付与が追加で必要。手順は
  `add-portfolio-item` スキル参照

### 件数表記は手で書き換えない

件数は **SEO description / Twitter description / サイドバーの `nav-count` / 各セクションの
`section-count`** の5系統に散っている。手で合わせようとすると必ずどこかが残る。
